...
def main_metas():
    ...
    gastos = col1.number_input("Gastos fixos", min_value=0.,format="%.2f")
    ...
    default_selic = selic_gov[filter_selic_date]["MetaSelic"].iloc[0]
    selic = st.number_input("Selic", min_value=0., value=default_selic, format="%.2f")
    selic_ano = selic / 100
    ...
    mensal = salario_liquido - gastos + rendimento_mes
    anual = 12 *(salario_liquido - gastos) + rendimento_ano
    with col1_pot.container(border=True):

        st.markdown(f"**Potencial Arrecadação Mês**: R$ {mensal:.2f}",
                    help = f"{salario_liquido:.2f} + (-{gastos:.2f}) + {rendimento_mes:.2f}")
    with col2_pot.container(border=True):
        st.markdown(f"""**Potencial Arrecadação Ano**: R$ {anual:.2f}""",
                    help=f"12 * ({salario_liquido:.2f} + (-{gastos:.2f})) + {rendimento_ano:.2f}")

    with st.container(border=True):
        col1_meta, col2_meta = st.columns(2)
        with col1_meta:
            meta_estipulada = st.number_input("Meta Estipulada",min_value=-999999999., format="%.2f", value=anual)

        with col2_meta:
            patrimonio_final = meta_estipulada + valor_inicio
            #patrimonio_final = patrimonio_final + (patrimonio_final * selic_ano) 
